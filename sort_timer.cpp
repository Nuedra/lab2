...
long measure_sort_time(ISorter<person>& sorter,
    ...
    std::cout << "Sort " << sequence.get_length() << " persons during the " << duration
              << " ms by " << name << std::endl;
...
void measure_and_save_sort_times_for_big() {
    std::string filename = "../csv/sort_times_big.csv";
    std::string png_filename = "../plots/sort_times_big.png";
    ...
    for (int size = step; size <= step * 10; size += step) {
        generate_and_write_persons_to_file(size);
        std::string input_filename = "../csv/data.csv";
